...
def detect_and_predict_mask(frame, faceNet, maskNet):
    ...
    # initialize our list of faces, their corresponding locations, and predictions
    faces = []
    locs = []
    preds = []
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            # extract the face ROI, convert it from BGR to RGB channel ordering, resize, and preprocess it
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        label = "Use Mask" if mask > withoutMask else "Without Mask"
        color = (0, 255, 0) if label == "Use Mask" else (0, 0, 255)
